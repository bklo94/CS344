...
int main(int argc, char **argv){
  ...
  char *keyBuffer = NULL, *plaintextBuffer = NULL, *host = "localhost", *auth = "decode#", *c;
...
//https://stackoverflow.com/questions/9206091/going-through-a-text-file-line-by-line-in-c
  ...
  *c = '&';
  ...
  //gets the port from argv, contains the code for the address family, then convert to network byte order, and then grab the IP address w
  port = atoi(argv[3]);
  ...
  //get response from server and put into keySize and then print the returned data
  keySize = getSockMessage(sockDesc, &keyBuffer);
  printf("%s\n", keyBuffer);
